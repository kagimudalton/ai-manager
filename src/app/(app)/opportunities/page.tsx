...
const TYPES = ["all", "fellowship", "internship", "hackathon", "scholarship", "grant"];
...
export default function OpportunitiesPage() {
  ...
  async function setStatus(id: string, status: "saved" | "dismissed") {
    ...
    setOpportunities((current) =>
      status === "dismissed"
        ? current?.filter((o) => o.id !== id) ?? null
        : current?.map((o) => (o.id === id ? { ...o, status } : o)) ?? null
    );
    try {
  ...
  if (error && !opportunities) return <ErrorState message={error} onRetry={load} />;
  if (!opportunities) return <LoadingState label="Loading opportunities..." />;
    ...
    <main className="max-w-md sm:max-w-xl md:max-w-3xl mx-auto px-4 sm:px-6 pt-2 pb-24">
      <div className="relative rounded-3xl overflow-hidden h-32 sm:h-40 mb-5 animate-fadeInUp">
        ...
        <div className="relative h-full flex flex-col justify-end p-5">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-white">Matched to your goals</h1>
        </div>
      </div>
      ...
      {visible.length === 0 ? (
        ...
        <div className="space-y-3">
          {visible.map((o, i) => {
            ...
            return (
              <Card key={o.id} className="animate-fadeInUp" style={{ animationDelay: `${i * 70}ms` }}>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button onClick={() => setStatus(o.id, "saved")}>
                    <Bookmark
                      size={18}
                      style={o.status === "saved" ? { color: "var(--brass)", fill: "var(--brass)" } : { color: "var(--muted)" }}
                    />
                  </button>
                </div>

                {o.deadline && <p className="text-xs text-muted mt-2">Due {new Date(o.deadline).toLocaleDateString()}</p>}
                ...
                >
                  <span>{expanded ? "Hide details" : "View eligibility & details"}</span>
                ...
                {expanded && (
                  <div className="mt-3 space-y-2 text-sm text-muted">
                    ...
                    {o.eligibility && (
                      <p>
                        <span className="font-medium text-text">Eligibility: </span>
                        {o.eligibility}
                      </p>
                    )}
                  </div>
